    ...
    engine.prepareWorldData(48, 48, function() {
        ...
        setTimeout(function() {
            ...
            entity.moveTo(1, 8, 5000, function() {
                entity.rotate(-90, 1000, 24, 24, function() {
                    entity.moveTo(8,8, 5000, function() {
                        var stopTime = new Date();
                        var delta = (stopTime.getTime() - startTime.getTime());
                        q.ok( 10900 < delta && delta < 11100, "Animated sprite moved in about 11 seconds (" + delta + ")");
                        q.start();
                    })
                });
            });
...
q.asyncTest('Moving 50 aminated sprites on map', 50, function() {
    engine.prepareWorldData(48,48, function(){
        engine.createScene('scene1');
        engine.showScene('scene1');
        var i,j, entities = [];
            ...
            for( n=0; n < entities.length; n++){
                (function(){
                    ...
                    entity.moveTo(entities[n].finish.x, entities[n].finish.y, 5000, function() {
                        var stopTime = new Date();
                        var delta = (stopTime.getTime() - startTime.getTime());
                        q.ok( 4900 < delta && delta < 5100, "Animated sprite moved in about 5 seconds (" + delta + ")");
                    });
                })();
            }
            setTimeout(function(){q.start();}, 6000);
...
q.asyncTest('Moving 50 aminated sprites on animated map', 50, function() {
...
q.asyncTest('Moving 50 aminated sprites on animated background', 50, function() {
...
q.asyncTest('Moving 50 aminated sprites on animated background and static map', 50, function() {
...
q.asyncTest('Move 2 large non-animated objects', 4, function() {
    engine.prepareWorldData(48,48, function() {
       engine.createScene('scene1');
       engine.showScene('scene1');
       setTimeout(function() {
           ...
           clouds1.moveTo(10, 10, 5000, function() {
               var stopTime = new Date();
               var delta = (stopTime.getTime() - startTime.getTime());
               q.ok( 4900 < delta && delta < 5100, "Gray large object moved in about 5 seconds (" + delta + ")");
           });
           clouds2.moveTo(0, 0, 5000, function() {
               var stopTime = new Date();
               var delta = (stopTime.getTime() - startTime.getTime());
               q.ok( 4900 < delta && delta < 5100, "Blue large object moved in about 5 seconds (" + delta + ")");
           });
           setTimeout(function() {
               clouds1.setZ(2);
               clouds2.setZ(1);
...
q.asyncTest('Rotate 50 small objects over map', 50, function() {
    engine.prepareWorldData(48, 48, function() {
            ...
            for (i = 0; i < entities.length; i++) {
                (function() {
                    ...
                    player.rotate(entities[i].theta,
                        ...
                        q.ok(4900 < delta && delta < 5100, 'Small object rotated in about 5 seconds (' + delta + ')');
                    });
                    setTimeout(function() {q.start();}, 6000);
                })();
            }
...
q.asyncTest('Rotate 2 large objects over map', 2, function() {
    engine.prepareWorldData(48, 48, function() {
        engine.createScene('sc1');
        engine.showScene('sc1');
        setTimeout(function() {
            ...
            cloud1.rotate(360, 5000, 240, 120, function() {
                var stopTime = new Date(),
                    delta = stopTime.getTime() - startTime.getTime();
                q.ok(4900 < delta && delta < 5100, 'Large object rotated in about 5 seconds (' + delta + ')');
            });
...
q.asyncTest('Show texts', 3, function() {
    ...
    setTimeout(function() {
        ...
        setTimeout(function() {
            q.ok($('*:contains("' + textLeft.text + '"):last').text() === textLeft.text, 'Text aligned left found');
            q.ok($('*:contains("' + textCenter.text + '"):last').text() === textCenter.text, 'Centered text found');
            q.ok($('*:contains("' + textRight.text + '"):last').text() === textRight.text, 'Text aligned right found');
            q.start();
        }, 500);
    }, 1000);
});